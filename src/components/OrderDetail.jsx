...
export default function OrderDetail({ orderId, onBack }) {
  ...
  const loadOrder = async () => {
    try {
      setLoading(true);
      const response = await fetch(`/api/webapp/orders/${orderId}`);
      if (!response.ok) {
  ...
  return (
    <div>
      ...
      <div className="space-y-4 px-4 pt-4 pb-24">
        ...
        {/* Manzil */}
        <div className="rounded-lg border border-slate-200 bg-white p-4">
          <h3 className="mb-2 text-sm font-medium text-slate-700">Yetkazib berish manzili</h3>
          <p className="text-slate-900">{order.address}</p>
          {order.location_lat && order.location_long && (
            <p className="mt-1 text-xs text-slate-500">
              📍 {Number(order.location_lat).toFixed(6)}, {Number(order.location_long).toFixed(6)}
            </p>
          )}
        </div>
        ...
        {/* Mahsulotlar */}
        <div className="rounded-lg border border-slate-200 bg-white p-4">
          <h3 className="mb-3 text-sm font-medium text-slate-700">Mahsulotlar</h3>
          <div className="space-y-2">
            {order.OrderItems && order.OrderItems.map((item) => (
              <div key={item.id} className="flex items-center justify-between border-b border-slate-100 pb-2 last:border-0">
                <div className="flex-1">
                  <p className="font-medium text-slate-900">{item.Product?.name || 'Mahsulot'}</p>
                  <p className="text-sm text-slate-500">{item.quantity} x {Number(item.price_at_purchase).toLocaleString()} so'm</p>
                </div>
                <p className="font-medium text-blue-600">
                  {(item.quantity * Number(item.price_at_purchase)).toLocaleString()} so'm
                </p>
              </div>
            ))}
          </div>
